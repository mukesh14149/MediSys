...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        System.out.println("Medical+login"+sharedread.getBoolean("Login_Status",false));

        if(sharedread.getBoolean("Login_Status",false)==true) {
       ...
       // else {
            ...
            sharedPref = context.getSharedPreferences("Login_details", Context.MODE_PRIVATE);
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
           ...
           // long itemId = cursor.getLong(
             //       cursor.getColumnIndexOrThrow(MediSysContract.MediSysEntry.COLUMN_NAME_EMAIL)
            //);
            Log.d("LoginActivity.java","Outside if");
            System.out.println(cursor.getCount());
            if( cursor.getCount()>0) {
                cursor.moveToNext();
                String pass = cursor.getString(cursor.getColumnIndex(MediSysContract.MediSysEntry.COLUMN_NAME_PASSWORD));
                Log.d("LoginActivity.java","I am in background");
                System.out.println("background_login"+pass);


                if (pass.equals(mPassword) == false)
                    return false;
                else{
                  SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
                    SharedPreferences.Editor editor = sharedPref.edit();
                    editor.putBoolean("Login_Status", true);
                    editor.putString("email_id",mEmail);
                    editor.commit();
